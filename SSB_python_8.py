...
class TXFlowgraph(gr.top_block):
    def __init__(self):
        ...
        # Audio source (live audio at 48 kHz)
        self.audio_source = audio.source(48000, "", True)
        
        # Audio gain to boost input amplitude
        self.audio_gain = blocks.multiply_const_ff(2.0)
        
        # SSB filter (USB, 300-3600 Hz)
        taps = firdes.complex_band_pass(
            gain=1.0,              # Gain: 1 (unchanged)
        ...
        self.resampler = filter.rational_resampler_ccf(
            interpolation=500,     # Adjusted for 8 Msps
            decimation=3
        )
        
        # HackRF sink
        self.sink = soapy.sink("driver=hackrf", "fc32", 1, "", "", [""], [""])
        self.sink.set_sample_rate(0, 8e6)  # 8 Msps
        self.sink.set_gain(0, "AMP", 14)   # Preamp on
        self.sink.set_gain(0, "VGA", 40)   # Reduced VGA gain to 40 dB for safety
        
        # Connections
        self.connect((self.audio_source, 0), (self.audio_gain, 0))
        self.connect((self.audio_gain, 0), (self.ssb_filter, 0))
...
class RXFlowgraph(gr.top_block):
    def __init__(self):
        ...
        self.source.set_sample_rate(0, 8e6)  # Updated to 8 Msps
        self.source.set_gain(0, "AMP", 0)
        self.source.set_gain(0, "LNA", 16)
        self.source.set_gain(0, "VGA", 54)

        # Early decimation to exactly 48 kHz
        self.resampler = filter.rational_resampler_ccf(
            interpolation=3,
            decimation=500  # 8e6 * 3 / 500 = 48000 Hz
        )
    ...
    while True:
        ...
        try:
            while True:
                ...
                if data == "f" or data in ["f VFOA", "f VFOB"]:
                ...
                elif data.startswith("F ") or data.startswith("F VFOA ") or data.startswith("F VFOB "):
                    try:
                        parts = data.split()
                        freq_index = 1 if len(parts) == 2 else 2 if len(parts) == 3 and parts[1] in ["VFOA", "VFOB"] else -1
                        if freq_index == -1:
                ...
                elif data == "s" or data == "s VFOA":
                    client_socket.send(b"0\r\nVFOA\r\n")
                    print("Sent split: 0, VFO: VFOA")
                elif data == "S VFOA 0 VFOA":
                    #client_socket.send(b"RPRT -1\r\nRPRT -1\r\n")
                    client_socket.send(b"RPRT 0\r\nRPRT 0\r\n")
                    print("Sent RPRT 0 RPRT 0 for invalid command 'S VFOA 0 VFOA'")
                elif data == "m VFOA":
                    client_socket.send(b"USB\r\n3000\r\n")
                    print("Sent mode: USB, bandwidth: 3000")
                elif data == "t" or data == "t VFOA":
                    ...
                    try:
                        ...
                        break
                elif data == "T 1" or data == "T VFOA 1" or data == "T VFOA 3":
                    try:
                        if not tx_active:
                            active_fg.stop()
                            active_fg.wait()
